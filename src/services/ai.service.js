...
export const searchPerfume = async (searchText) => {
  ...
  if (Object.keys(result).length !== 0) {
    ...
    const openai = new OpenAI({
      apiKey: zprocess.env.OPENAI_API_KEY,
    });
...
function cosineSimilarity(vecA, vecB) {
  ...
  // 각 벡터의 크기 계산
  const magnitudeA = Math.sqrt(vecA.reduce((acc, val) => acc + Math.pow(val, 2), 0));
  const magnitudeB = Math.sqrt(vecB.reduce((acc, val) => acc + Math.pow(val, 2), 0));
...
export const recommendPerfumeUser = async (userId) => {
  ...
  const allPerfumes = allPerfumesResult.perfumes.map((perfume) => perfume.Description);

  // 사용자가 좋아하는 향수의 벡터 생성
  const userLikesVector = userLikes.map((description) => {
    const tokens = description.split(" ");
    return allPerfumes.map((perfumeDescription) => (tokens.some((token) => perfumeDescription.includes(token)) ? 1 : 0));
  });

  // 전체 향수 데이터베이스의 벡터 생성
  const allPerfumesVector = allPerfumes.map((perfumeDescription) => {
    const tokens = perfumeDescription.split(" ");
    return userLikes.map((userLike) => (tokens.some((token) => userLike.includes(token)) ? 1 : 0));
  });
  ...
  const recommendedPerfumeIndices = similarities
    ...
    .filter((item) => !likedPerfumeIds.includes(allPerfumesResult.perfumes[item.index].PerfumeID)) // 이미 사용자가 찜한 향수 제외
    .slice(0, 3) // 가장 유사한 향수 3개 추출
    .map((item) => item.index);

  // 추천된 향수 가져오기
  const recommendedPerfumes = recommendedPerfumeIndices.map((index) => allPerfumesResult.perfumes[index]);
